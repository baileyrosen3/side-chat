...
| --- | --- |
| Omarchy interface | A regular bar widget (`Panel.qml`) with `Ui.KeyboardPanel`, native placement/dismissal/focus, and theme borders. `ChatWindow.qml` is reusable content, so Island can embed it. `Main.qml` owns one shared backend; `ChatBridge.js` connects per-monitor widgets regardless of load order. |
