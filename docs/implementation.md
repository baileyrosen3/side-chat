...
| Local speech | Parakeet Unified recognition reusing the downloaded Voxtype model, an optional file-backed Voxtype recognition bridge, streaming Pocket TTS, optional Kokoro output and legacy recognition, live captions, microphone/playback indicators, push-to-talk, hands-free listening, and optional upstream “Hey Jarvis” wake/follow-up listening for Peek. |
...
The setup used during development has **Oh My Pi** as the default agent, **Parakeet Unified English 0.6B** for recognition, and **Pocket TTS with Javert** for replies. A Voxtype mode can instead use the system daemon for recognition, leaving Peek as the assistant/TTS layer and avoiding a duplicate ASR model in RAM. Peek currently follows the system-default microphone in bundled mode. These are configurable; they do not replace other users’ agent or audio preferences. Speech runs locally without a paid speech service. The chosen CLI/provider keeps its own license, account requirements, and any model costs; see [THIRD_PARTY.md](../THIRD_PARTY.md).
