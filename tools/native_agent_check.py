...
agent=sys.argv[1];folder=Path(tempfile.mkdtemp(prefix='jarvis-'+agent+'-live-'))
...
b.jarvis.prefs.update(handsFree=False,muted=True)
def wait():
 ...
 while b.busy and time.monotonic()<deadline:
  for prompt in list(b.ui_requests):
   ...
   if 'jarvis' in title.lower() and 'computer' in title.lower():
...
try:
 ...
 Path('/tmp/jarvis-'+agent+'-live.json').write_text(json.dumps(result,indent=2))
