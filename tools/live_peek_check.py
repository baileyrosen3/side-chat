...
folder=Path(tempfile.mkdtemp(prefix='peek-live-'))
...
b.peek.prefs.update(handsFree=False,muted=True,screenContext='off')
try:
 b.peek.ensure_control();b.peek.state['enabled']=True
 b.dispatch({'action':'send','text':'In this disposable test folder, change fixture.conf from font_size = 11 to font_size = 12 using peek_computer config_read and config_write with expected sha256, then read it back. Also call peek_computer with op windows once to verify the desktop tool is available. Do not focus, click, type, or change any other files. Briefly report the result.'})
