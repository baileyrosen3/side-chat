...
folder=Path(tempfile.mkdtemp(prefix='jarvis-steer-live-'));(folder/'fixture.conf').write_text('size=0\n')
b=Bridge(folder/'state',lambda _:None);b.settings['cwd']=str(folder);b.jarvis.prefs.update(handsFree=False,muted=True,screenContext='off');b.jarvis.state['enabled']=True
try:
 text='This is a disposable steering test. First run a shell command that writes first to progress.txt and sleeps for 5 seconds. Only after that command finishes, use jarvis_computer config_read/config_write to change fixture.conf to size=11 followed by a newline, then verify it. Do not touch any other files or apps. Briefly report the result.'
 ...
 session=b.current['native']['sessionFile'];b.jarvis.steer('Change the requested value to size=12 instead of size=11. Keep the existing progress.txt.')
 deadline=time.monotonic()+70
 while time.monotonic()<deadline:
  if not b.busy and not b.jarvis.pending:break
  time.sleep(.1)
 assert not b.busy
