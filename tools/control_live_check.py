...
try:
 ...
 action('click','entry')
 action('type','entry',text='Peek ✓ café')
 action('key','entry',key='Ctrl+a')
 action('type','entry',text='Verified Unicode ✓')
 action('click','button')
 action('scroll','scroll',amount=-5)
 frame=c.handle({'op':'screenshot','screen':monitor['name']})['details'];target=next(w for w in hypr('clients') if w.get('title')=='Peek control fixture')
